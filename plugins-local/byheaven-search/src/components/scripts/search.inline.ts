...
// BYHEAVEN CJK tokenizer (ported from the v4 self-written search):
// CJK has no word delimiters, so the upstream char-only encoder indexes each
// CJK char as an isolated token. That keeps recall but loses precision: a
// multi-char query like "愿景" then matches any document containing 愿 and 景
// anywhere, even non-adjacent (false positives). We additionally emit a bigram
// for every pair of ADJACENT CJK chars, so an exact two-char word ranks for
// the bigram token and only documents where the chars are adjacent match the
// word. Latin/whitespace handling is unchanged from upstream.
const encoder = (str: string): string[] => {
  ...
  let i = 0;
  let prevCJKChar: string | null = null;
  ...
  for (const char of lower) {
    ...
    if (isCJK) {
      ...
      tokens.push(char);
      // Emit the adjacent-pair bigram (prev + current) for precision.
      if (prevCJKChar !== null) {
        tokens.push(prevCJKChar + char);
      }
      prevCJKChar = char;
