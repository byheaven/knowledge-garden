...
// Mirrors the official quartz-community plugin template's inline-script loader:
//   - .scss files are compiled to plain CSS via sass, then handed back as text
//     so they can be assigned to Component.css.
//   - .inline.ts files are transpiled (and bundled/minified) to browser ESM via
//     a nested esbuild build, then handed back as text so they can be assigned
//     to Component.beforeDOMLoaded / .afterDOMLoaded or a transformer's inline
//     JSResource. This strips all TypeScript syntax that would break in the
//     browser.
